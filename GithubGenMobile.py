...
USERNAME_PREFIX = "dev"  # Added prefix

class GithubMobileGenerator:
    """GitHub account generator for mobile version."""
    ...
    def __get_email_address(self, level: int = 0):
        ...
        try:
            ...
            email_address = self.email_service.get_email(level=level+1)
            ...
            # Save token if available, similar to Instagram script
    ...
    def __launch_browser(self, level: int = 0):
        ...
        try:
            ...
            if os.path.exists(BRAVE_PATH):
                ...
                logger(f"Using Brave Browser: {BRAVE_PATH}", level=level+1)
            else:
                logger(f"⚠ Brave Browser not found at {BRAVE_PATH}, using default Chromium", level=level+1)
    ...
    def run_flow(self, level: int = 0):
        ...
        print("  ")
        logger("GitHub Mobile Account Generator", level=level)
    ...
    def __handle_captcha(self, level: int = 0):
        ...
        try:
            ...
            for round_idx in range(max_rounds):
                ...
                question_selector = "#root > div > div > div > h2 > span" # Updated as per user last edit
                # note: user changed it to just span, removing strong:nth-child(2)
...
if __name__ == "__main__":
    ...
    generator = GithubMobileGenerator(use_tor=True)

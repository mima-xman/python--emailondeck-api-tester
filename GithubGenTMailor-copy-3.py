...
from utils import logger, format_error, renew_tor, get_current_ip, get_2fa_code
...
class GithubTMailorGenerator:
    ...
    def human_type(self, selector, text, min_delay_ms=50, max_delay_ms=200):
        ...
        try:
             self.page.type(selector, text, delay=random.randint(min_delay_ms, max_delay_ms), timeout=60000)
    ...
    def __get_email_address(self, level: int = 0):
        ...
        try:
            # self.email_service = TempMailExtensionAPI()
            # self.email_service = TMailorAPI()
            # self.email_service = TempMailIOAPI()
            self.email_service = EmailOnDeck()
            
            # Pass level + 1 if the method accepts it, otherwise it might print directly
            username = self.account_data.get('username', None)
            result = self.email_service.generate_email() # custom_name=username
            ...
            try:
                ...
                while iframe1_exists:
                    ...
                    if number_times_iframe1_steel_exists > 5:
                        self.natural_delay()
                        if self.__check_puzzle_displayed(level=level+1):
                            return False
                ...
                if new_email:
                    ...
                    if content:
                        # logger("-"*50, level=level+2)
                        # logger(f"Email content: {content[self.email_service.body_key]}", level=level+2)
                        # logger("-"*50, level=level+2)

                        match = re.search(r'>\s*(\d{8})\s*</span>', content[self.email_service.body_key])
                        if match:
                            code = match.group(1)
                ...
                try:
                    ...
                    if current_url != GITHUB_SIGNUP_URL:
                        logger("✓ Code auto submited", level=level+2)
                        pass
                    else:
                        raise Exception("✗ Code not auto submited")
                ...
                # Save account data
                # self.__save_account_data(level=level+1)

                # =======================
                # 2FA Setup Flow
                # =======================
                if not self.__setup_2fa(level=level+1):
                    logger("✗ 2FA Setup failed", level=level+1)
                    # We continue even if 2FA fails, or we can return False. 
                    # Usually better to try to save what we have.
                else:
    ...
    def __save_account_data(self, level: int = 0):
        try:
            account_data = {
            ...
            with open(f"{OUTPUT_DIR}/account_{current_time}.json", "a", encoding="utf-8") as f:
                f.write(json.dumps(account_data, indent=4))
                logger(f"✓ Account data saved: {OUTPUT_DIR}/account_{current_time}.json", level=level+2)
        except Exception as e:
            logger(f"✗ An error occurred on save account data: {format_error(e)}", level=level+2)
    ...
    def __check_puzzle_displayed(self, level: int = 0):
        try:
            # Select iframe 1
            iframe1_selector = "#captcha-container-nux > div > div > div:nth-child(3) > iframe"
            logger(f"Waiting for iframe1: {iframe1_selector}...", level=level+1)
            self.page.wait_for_selector(iframe1_selector, state='attached', timeout=30000)
            ...
            if iframe3.locator(puzzle_btn_selector).is_visible(timeout=5000):
                logger("✓ Puzzle found", level=level+1)
                return True
    ...
    def __login(self, level: int = 0):
        print("  ")
        logger("[######] Logging in...", level=level)
        try:
            # 1. Fill Email
            email_selector = "#login_field"
            # We use username or email
            login_val = self.account_data.get('username')
            if not login_val:
                login_val = self.account_data.get('email_address')
            
            if not self.__wait_and_fill(email_selector, login_val, "Login Field", level=level+1):
                return False
    ...
    def __setup_2fa(self, level: int = 0):
        print("  ")
        ...
        try:
            ...
            for i in range(10):
                ...
                if not current_url.startswith("https://github.com/login") and not current_url.startswith("https://www.github.com/login"):
                    logger("⚠ Not on login page, checking if we need to click anything...", level=level+1)
                    if i == 9:
                        return False
                    self.natural_delay(2, 4)
                else:
            ...
            # 1. Login
            if not self.__login(level=level+1):
                return False
            ...
            for i in range(10):
                try:
                    ...
                    logger("⚠ Not on dashboard yet, checking if we need to click anything...", level=level+1)
                    self.natural_delay(2, 4)
                    if i == 9:
                        return False
            ...
            # 3. Like a human scroll behavior
            ...
            settings_selector = "#__primerPortalRoot__ > div > div a[href='/settings/profile']"
            ...
            # 7. Click Password and authentication
            ...
            # 9. Click Enable 2FA (Start setup)
            logger("Clicking Enable 2FA...", level=level+1)
            self.human_click("a[href='/settings/two_factor_authentication/setup/intro']")
            self.natural_delay()

            # # 10. Click Setup using app
            # logger("Clicking view secret...", level=level+1)
            # self.human_click("#dialog-show-two-factor-setup-verification-mashed-secret")
            # self.natural_delay()

            # 11. Get two-factor secret
            secret_selector = "#two-factor-setup-verification-mashed-secret > scrollable-region > div > div, [data-target='two-factor-setup-verification.mashedSecret']"
            ...
            # 14. Click Continue
            continue_btn_selector = "#wizard-step-factor > div > div > div > div > div > button[data-action='click:single-page-wizard-step#onNext']"
            logger("Clicking Continue...", level=level+1)
            self.human_click(continue_btn_selector)
            
            self.natural_delay()

            # 15. Check for recovery codes
            ...
            try:
                ...
                logger("⚠ Recovery codes not immediately visible, maybe manual click needed again?", level=level+1)
            
            self.natural_delay()

            # 16. Get recovery codes
            ...
            # 18. Click I have saved my recovery codes
            logger("Clicking 'I have saved my recovery codes'...", level=level+1)
            self.human_click("button[data-target='single-page-wizard-step.nextButton']:has-text('I have saved my recovery codes')")
            self.natural_delay()
            ...
            for i in range(max_retries):
                try:
                    ...
                    if i < max_retries - 1:
                        ...
                        success, self.ip = renew_tor(self.tor_control_port, self.tor_port, level=level+1)

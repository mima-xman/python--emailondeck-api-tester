...
class InstagramMobileGenerator:
    ...
    def __wait_for_verification_code(self, level: int = 0):
        ...
        while time.time() - start_time < MAX_WAIT_TIME_FOR_VERIFICATION_CODE:
            messages = self.email_service.get_messages(level=level+1)
            
            if messages:
                msg = messages[0]
                logger(f"✓ Email received: {msg.subject}", level=level+1)
                ...
                if match:
                    ...
                    content = self.email_service.get_content(msg.msg_id, level=level+1)
